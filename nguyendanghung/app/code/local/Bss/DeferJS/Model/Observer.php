...
class Bss_DeferJS_Model_Observer 
{
   public function deferScript($observer) {
      ...
      $conditionalJsPattern = '#(<\!--\[if[^\>]*>\s*<script.*</script>\s*<\!\[endif\]-->)|(<\!--\s*<script(?! nodefer).*</script>\s*-->)|(<script(?! nodefer).*</script>)#isU';
        // lấy ra tất cả các thẻ js
       preg_match_all($conditionalJsPattern, $html, $_matches);
       // biến mảng thành chuỗi chỉ lấy phần tử 0 trong array (cac file js).
      $_js_if = implode('', $_matches[0]);
//        var_dump($_js_if);
//       die();
       // replace $html chỗ nào có script thì cho bằng rỗng.
...
//       die();
      ...
      if(Mage::getStoreConfig('deferjs/general/show_path')) {
         ...
         $h = '<table cellspacing="0" cellpadding="2" border="1" attr="bss" style="width:auto;background-color:white">
         <tbody>
            <tr>
               <th>Module</th>
               <th>Controller</th>
